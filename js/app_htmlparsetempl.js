...
`
// note line 12: if patent[1] (link to file) doesn't exist then just put in a page-reload # hyperlink
// note line 18: if patent[4] (claim text) contains 'dependent' then addClass 'claim-dependent' to the summary
...
function generateOutput(results, callback) {
    try {
       if (typeof results[0][7] === 'number') {
         // results[n][7] is a claimID in the mainView Case
         return callback(null, table(results), results.length.toString());
       } else {
         // results[n][7] is a Path to the ruling
